...
class AdditionnameController extends BaseController {
  getAll = async (req, res, next) => {
    const model = await AdditionnameModel.findAll()
    res.send(model)
  }
  create = async (req, res) => {
    const {name} = req.body
    try{
      const model = await AdditionnameModel.create({
        name
      })
  ...
  update = async (req, res) => {
    ...
    const {name} = req.body
    const model = await AdditionnameModel.findOne({
      where: {id: id}
    })
...
module.exports = new AdditionnameController;