const UnityModefa = require('../models/unity.model')
...
class UnityController extends BaseController {
  getAll = async (req, res, next) => {
    const model = await UnityModefa.findAll()
    res.send(model)
  }
  create = async (req, res) => {
    const {name} = req.body
    try{
      const model = await UnityModefa.create({
        name
      })
  ...
  update = async (req, res) => {
    ...
    const {name} = req.body
    const model = await UnityModefa.findOne({
      where: {id: id}
    })
