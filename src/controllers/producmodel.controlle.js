const ProducmodelMOdel = require('../models/product_model.model')
...
class ProductmodelController extends BaseController {
  getAll = async (req, res, next) => {
    const model = await ProducmodelMOdel.findAll()
    res.send(model)
  }
  create = async (req, res) => {
    const {name} = req.body
    try{
      const model = await ProducmodelMOdel.create({
        name
      })
  ...
  update = async (req, res) => {
    ...
    const {name} = req.body
    const model = await ProducmodelMOdel.findOne({
      where: {id: id}
    })
...
module.exports = new ProductmodelController;